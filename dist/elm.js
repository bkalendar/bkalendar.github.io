...
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		{events: _List_Nil, readyToDownload: false, result: _List_Nil},
		$elm$core$Platform$Cmd$none);
};
...
var $author$project$Class$Class = F7(
	function (title, desp, room, wday, start, end, weeks) {
		return {desp: desp, end: end, room: room, start: start, title: title, wday: wday, weeks: weeks};
	});
...
var $author$project$Class$weekParser = A2(
		...
		return $elm$parser$Parser$oneOf(
			_List_fromArray(
				[
					...
					$elm$parser$Parser$symbol('|')),
					$elm$parser$Parser$succeed(
					$elm$parser$Parser$Done(result))
				]));
	});
var $author$project$Class$parser = function () {
	var toClass = F8(
		function (id, display, group, wday, start, end, room, weeks) {
			var title = display + (' - ' + id);
			var desp = group;
			return A7($author$project$Class$Class, title, desp, room, wday, start, end, weeks);
		});
		...
		A2(
					...
					A2(
						...
						A2(
							...
							A2(
								...
								A2(
									$elm$parser$Parser$keeper,
									$elm$parser$Parser$succeed(toClass),
									A2(
										$elm$parser$Parser$ignorer,
			...
			A2(
				...
				A2(
					...
					cellChomper),
				$elm$parser$Parser$symbol('\t'))),
		$author$project$Class$weekParser);
}();
...
var $author$project$Class$parse = function (raw) {
	return $elm$core$Result$toMaybe(
		A2($elm$parser$Parser$run, $author$project$Class$parser, raw));
};
var $elm$core$Basics$min = F2(
	function (x, y) {
		return (_Utils_cmp(x, y) < 0) ? x : y;
	});
var $author$project$Class$monthToInt = function (month) {
	switch (month.$) {
		case 'Jan':
			return 1;
		case 'Feb':
...
var $author$project$Class$origin = 1613944800000;
...
var $author$project$Class$toDate = F3(
	function (wday, start, week) {
		var dttm = A2(
					...
					_List_fromArray(
						[
							...
							[
								...
								$author$project$Class$monthToInt),
		...
		var date = ($author$project$Class$origin + (week * 604800000)) + ((wday - 2) * 86400000);
		var startTime = $elm$time$Time$millisToPosix(date + (start * 3600000));
		return timestamp(startTime);
	});
var $author$project$Class$toEvent = F2(
	function (uuid, _class) {
		var _v0 = _class.weeks;
		if (!_v0.b) {
			return '';
		} else {
			var headWeek = _v0.a;
			var tailWeeks = _v0.b;
			var firstWeek = A3($elm$core$List$foldl, $elm$core$Basics$min, headWeek, tailWeeks);
			return 'BEGIN:VEVENT' + ('\nUID:' + (uuid + ('\nDTSTAMP:20210516T200000' + ('\nSUMMARY:' + (_class.title + ('\nDESCRIPTION:' + (_class.desp + ('\nLOCATION:' + (_class.room + ('\nDTSTART:' + (A3($author$project$Class$toDate, _class.wday, _class.start, firstWeek) + ('\nDTEND:' + (A3($author$project$Class$toDate, _class.wday, _class.end + 1, firstWeek) + ('\nRDATE:' + (A2(
				$elm$core$String$join,
				',',
				A2(
					$elm$core$List$map,
					A2($author$project$Class$toDate, _class.wday, _class.start),
					_class.weeks)) + '\nEND:VEVENT')))))))))))))));
		}
	});
...
var $author$project$Main$update = F2(
	function (msg, model) {
		if (msg.$ === 'GotInput') {
			...
			var newResult = A2(
				$elm$core$List$filterMap,
				$author$project$Class$parse,
				A2($elm$core$String$split, '\n', raw));
			var eventGenerator = function (_class) {
				return A2(
					$elm$random$Random$map,
					function (uuid) {
						return A2(
							$author$project$Class$toEvent,
			...
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{events: _List_Nil, readyToDownload: false, result: newResult}),
				$elm$core$Platform$Cmd$batch(
					A2(
						$elm$core$List$map,
						function (_class) {
							return A2(
								$elm$random$Random$generate,
								$author$project$Main$GotEvent,
								eventGenerator(_class));
						},
						newResult)));
		} else {
			var event = msg.a;
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{
						...
						readyToDownload: _Utils_eq(
							$elm$core$List$length(model.result),
							$elm$core$List$length(model.events) + 1)
					}),
				$elm$core$Platform$Cmd$none);
		}
	});
...
var $author$project$Class$toCalendar = function (events) {
	return 'BEGIN:VCALENDAR\r\nVERSION:2.0\r\nPRODID:-//bkalendar//Google Calendar v1.0/VI\r\n' + (A2($elm$core$String$join, '\n', events) + '\r\nEND:VCALENDAR');
};
var $elm$core$Char$toUpper = _Char_toUpper;
var $author$project$Class$abbrHelp = F3(
	function (revChars, snatch, text) {
		abbrHelp:
		while (true) {
			...
			if (_v0.$ === 'Nothing') {
				...
				switch (_v0.a.a.valueOf()) {
					case '(':
						var _v1 = _v0.a;
						var rest = _v1.b;
						var $temp$revChars = A2(
							$elm$core$List$cons,
							_Utils_chr('('),
							revChars),
							$temp$snatch = true,
							$temp$text = rest;
						revChars = $temp$revChars;
						snatch = $temp$snatch;
						text = $temp$text;
						continue abbrHelp;
					case ')':
...
var $author$project$Class$abbr = function (name) {
	var result = A3($author$project$Class$abbrHelp, _List_Nil, true, name);
	return ($elm$core$String$length(result) === 1) ? A2(
		$elm$core$Maybe$withDefault,
		name,
		$elm$core$List$head(
			A2($elm$core$String$split, ' ', name))) : result;
};
var $elm$html$Html$h2 = _VirtualDom_node('h2');
var $author$project$Main$viewPreview = function (classes) {
	var filterWday = function (wday) {
		return A2(
			$elm$core$Basics$composeL,
			$elm$core$String$join(', '),
			$elm$core$List$filterMap(
				function (_class) {
					return _Utils_eq(_class.wday, wday) ? $elm$core$Maybe$Just(
						$author$project$Class$abbr(_class.title)) : $elm$core$Maybe$Nothing;
				}));
	};
	return A2(
		$elm$html$Html$div,
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('font-bold text-xl mt-4 mb-1')
					]),
				_List_fromArray(
					[
						$elm$html$Html$text('Xem trước:')
					])),
				A2(
				$elm$html$Html$p,
				_List_Nil,
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Thứ hai: ')
							])),
						$elm$html$Html$text(
						A2(filterWday, 2, classes))
					])),
				A2(
				$elm$html$Html$p,
				_List_Nil,
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Thứ ba: ')
							])),
						$elm$html$Html$text(
						A2(filterWday, 3, classes))
					])),
				A2(
				$elm$html$Html$p,
				_List_Nil,
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Thứ tư: ')
							])),
						$elm$html$Html$text(
						A2(filterWday, 4, classes))
					])),
				A2(
				$elm$html$Html$p,
				_List_Nil,
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Thứ năm: ')
							])),
						$elm$html$Html$text(
						A2(filterWday, 5, classes))
					])),
				A2(
				$elm$html$Html$p,
				_List_Nil,
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Thứ sáu: ')
							])),
						$elm$html$Html$text(
						A2(filterWday, 6, classes))
					])),
				A2(
				$elm$html$Html$p,
				_List_Nil,
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Thứ bảy: ')
							])),
						$elm$html$Html$text(
						A2(filterWday, 7, classes))
					])),
				A2(
				$elm$html$Html$p,
				_List_Nil,
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Chủ nhật: ')
							])),
						$elm$html$Html$text(
						A2(filterWday, 8, classes))
					]))
			]));
};
var $author$project$Main$view = function (model) {
	var textareaStyle = 'h-16 flex-none p-2 bg-gray-200 text-gray-500 w-full overflow-hidden';
	var invalidStyle = 'font-bold text-red-600';
	var downloadStyle = 'text-green-500 border-b-4 border-green-500';
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('w-1/3 mx-auto pt-16 min-h-screen flex flex-col')
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$Events$onInput($author$project$Main$GotInput),
						$elm$html$Html$Attributes$class(textareaStyle)
					]),
				_List_Nil),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('flex-grow h-full')
					]),
				model.readyToDownload ? _List_fromArray(
					[
						A2(
						$elm$html$Html$a,
						_List_fromArray(
							[
								...
								'data:text/calendar,' + $elm$url$Url$percentEncode(
									$author$project$Class$toCalendar(model.events))),
								$elm$html$Html$Attributes$class(downloadStyle),
								$elm$html$Html$Attributes$tabindex(0)
							]),
						_List_fromArray(
							[
								$elm$html$Html$text('click here to download')
							])),
						$author$project$Main$viewPreview(model.result)
					]) : _List_fromArray(
					[
						A2(
						$elm$html$Html$a,
						_List_fromArray(
							[
								$elm$html$Html$Attributes$href('#invalid-input'),
								$elm$html$Html$Attributes$class(invalidStyle)
							]),
						_List_fromArray(
							[
								$elm$html$Html$text('please check your input')
							]))
					])),
				A2(
				$elm$html$Html$p,
				...
				_List_fromArray(
					[
						$elm$html$Html$text('made with love by NDK')
					])),
				A2(
				$elm$html$Html$p,
				_List_fromArray(
				...
				_List_fromArray(
					[
						$elm$html$Html$text('facebook · github')
					]))
			]));
};
...
_Platform_export({'Main':{'init':$author$project$Main$main(
	$elm$json$Json$Decode$succeed(_Utils_Tuple0))({"versions":{"elm":"0.19.1"},"types":{"message":"Main.Msg","aliases":{},"unions":{"Main.Msg":{"args":[],"tags":{"GotInput":["String.String"],"GotEvent":["String.String"]}},"String.String":{"args":[],"tags":{"String":[]}}}}})}});
